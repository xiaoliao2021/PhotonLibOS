...
public:
    ...
    void resolve(std::string_view host, PDelegate<void, IPAddr> func) { func(resolve(host)); }
    // If filter callback returns false, the IP will be abandoned.
    virtual IPAddr resolve_filter(std::string_view host, PDelegate<bool, IPAddr> filter) = 0;
